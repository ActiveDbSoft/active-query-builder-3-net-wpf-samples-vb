     ...
     Partial Public Class CustomDataGrid
        ...
        Private Sub DataView_AutoGeneratedColumns(sender As Object, e As EventArgs)
            For Each dataGridColumn As DataGridColumn In DataView.Columns
                Dim data as HeaderData
                data = New HeaderData() With {
                    .Title = dataGridColumn.SortMemberPath
                    }
                dataGridColumn.Header = data

                Dim _sorting as SortedColumn= QueryTransformer.Sortings.FirstOrDefault(Function(sort) sort.Column.OriginalName = data.Title)

                If _sorting Is Nothing Then
                    Continue For
                End If

                data.Counter = QueryTransformer.Sortings.IndexOf(_sorting) + 1

                Select Case _sorting.SortType
